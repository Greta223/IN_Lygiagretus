...
def mergesort(lyst):  # It returns a copy of the array but already merged
    if len(lyst) <= 1:
        return lyst
    ind = len(lyst) // 2
    return merge(mergesort(lyst[:ind]),
                 mergesort(lyst[ind:]))  # it calls again itself to continue digging inside the array
...
def mergeSortParallel(lyst,
                      n):  # the main method of mergeSort in parallel, taking into acount the cores of your machine
    numproc = n
    # Divides the indices
